...
class Recipe {
    ...
    static async create(userId, recipeData) {
        ...
        try {
            ...
            // 2. Malzemeleri Kaydet (Burada ingredient_name kullanıyoruz)
           ...
           // 3. Besin Değerlerini Kaydet (Parametre sayısını düzelttik)
            if (nutrition && Object.keys(nutrition).length > 0) {
                await client.query(
                    `INSERT INTO recipe_nutrition (recipe_id, calories, protein, fats, carbs, fiber)
                    VALUES ($1, $2, $3, $4, $5, $6)`,
                    [
                        recipe.id, 
                        nutrition.calories || 0, 
                        nutrition.protein || 0, 
                        nutrition.fats || nutrition.fat || 0, // Hem fat hem fats destekler
                        nutrition.carbs || 0, 
                        nutrition.fiber || 0
                    ]
                );
    ...
    //get recipe by id with ingre and nutr.
    static async findById(id, userId){
        ...
        const ingredientsResult = await db.query(
            `SELECT ingredient_name as name, quantity, unit FROM recipe_ingredients WHERE recipe_id = $1`, [id]
        );
    ...
    RETURNING *`,
        [
            name, description, cuisine_type, difficulty, prep_time, 
            cook_time, servings, instructions, dietary_tags, 
            user_notes, image_url, id, userId
        ]
    );
    ...
    static async getStats(userId){
        const result = await db.query(
            `SELECT COUNT(*) as total_recipes,COUNT(DISTINCT cuisine_type) as cuisine_types_count, AVG(cook_time) as avg_cook_time FROM recipes WHERE user_id = $1`, [userId]
        );
